...
bash /osgeo/mapserver.org/scripts/build_docs.sh /osgeo/mapserver.org/docs-git branch-6-0 /osgeo/mapserver.org/htdocs/ >> /osgeo/mapserver.org/scripts/cron-logs/log-60-$rundate.txt 2>&1
rundate=`date +%Y%m%d-%H%M`
